...
INSERT INTO original_tags (file_id, distinct_name, make, model, height, width, focal_length, light, scene, macro)
SELECT DISTINCT
    sh.file_id,
    dp.distinct_name || extension AS distinct_name,
    image_make AS make,
...
FROM original_photos dp
JOIN sorted_height sh ON sh.file_id = dp.id
