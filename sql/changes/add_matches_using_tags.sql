...
WHERE gt.make = ot.make
    ...
    AND (gt.light = ot.light OR gt.light IS NULL)
    AND (gt.focal_length = ot.focal_length OR
        (gt.focal_length IS NULL AND ot.focal_length IS NULL))
    AND (gt.scene = ot.scene OR
        (gt.scene IS NULL AND ot.scene IS NULL))
    AND (gt.macro = ot.macro OR
        (gt.macro IS NULL AND ot.macro IS NULL))
